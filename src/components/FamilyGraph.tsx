  ...
  const nodes: Node[] = useMemo(() => {
    return members.map((m, i) => {
      ...
      return {
        ...
        data: { 
          label: (
            <div className={`p-4 rounded-2xl shadow-xl border-2 transition-all ${m.role === 'admin' ? 'bg-blue-900 border-blue-400 text-white' : 'bg-white border-slate-100 text-slate-900'}`}>
               <div className="flex items-center gap-2 mb-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${m.role === 'admin' ? 'bg-blue-500' : 'bg-slate-100'}`}>
                     {m.role === 'admin' ? <Shield className="w-4 h-4" /> : <User className="w-4 h-4 text-slate-500" />}
                  </div>
               ...
               <div className="flex justify-between items-center text-[8px] font-mono">
                  <span className="flex items-center gap-1"><Heart className="w-2 h-2 text-rose-500" /> SYNC</span>
                  <span className={`${isRevoked ? 'text-rose-500' : 'text-emerald-500'} flex items-center gap-1`}>
                    <Lock className="w-2 h-2" /> {isRevoked ? 'OFF' : 'LIVE'}
                  </span>
               </div>
            </div>
          )
